...
export function TripsList({ initialTrips }: { initialTrips: TripRow[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return initialTrips.filter(
      (t) =>
        t.from_text.toLowerCase().includes(q) ||
        t.to_text.toLowerCase().includes(q)
    );
  }, [initialTrips, query]);
      ...
      {filtered.length === 0 ? (
        ...
        <ul className="space-y-3">
          {filtered.map((t) => {
            ...
            const seatsLeft = Math.max(t.seats_total - t.seats_taken, 0);
            return (
              <li
                key={t.id}
              ...
              >
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">
                    {formatDateTime(t.depart_at)} · {seatsLeft} seats left · {firstName(
